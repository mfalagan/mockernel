...
`mockernel` is a formative OSdev project targeting the Raspberry Pi 5.
...
In practice, this means:
 ...
 - following the Raspberry Pi firmware boot flow and the AArch64 kernel entry contract.
...
v1 is a minimal but coherent Unix-shaped kernel. It boots in the target hardware, runs multiple user-mode processes in isolated address spaces, exposes a syscall interface for process lifecycle and file I/O, mounts an in-memory filesystem seeded from a compiled-in initramfs, and presents a shell prompt from which a user can run programs.
